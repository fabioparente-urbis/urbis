...
export default function MacPage() {
  ...
  /**
   * Execução do que o URBI propôs: quando o analista responde "Faz sentido" numa intervenção com
   * `acaoLocal`, o chat devolve este evento pra TELA fazer — só a tela tem os botões e o estado.
   * Hoje só exportação (leitura pura, não altera análise); qualquer ação que mude dado tem que
   * passar pelo aceite explícito do lugar onde ela mora, não por aqui.
   */
  useEffect(() => {
    function onAcao(e: Event) {
      ...
      /**
       * Backup completo em dois arquivos — "ele pode exportar os dois simultaneamente pegando
       * todo o LIP e TODAS AS ANÁLISES DO MAC... tudo, 2 arquivos de Excel" (Fábio, 08/09/2026).
       * Usa as MESMAS rotas dos botões da tela (nada novo, nada paralelo que possa divergir):
       * `exportar-lip` e `exportar-mac?todas=true`. Um respiro entre os dois porque navegador
       * costuma engolir o segundo download disparado no mesmo instante.
       */
      window.open(`/api/processo/exportar-lip?codigo=${encodeURIComponent(codigo)}&tipo=${tipoProcesso || "regularizacao"}`, "_blank");
      setTimeout(() => {
  ...
  /**
   * EXCLUI a análise — 08/09/2026, pedido do Fábio: "ao clicar na lixeira de uma análise, ela
   * conste como se nunca tivesse sido clicada, nunca iniciada, excluísse tudo dela".
   *
   * Antes isto só ZERAVA os campos: a linha continuava no banco, então o botão da análise seguia
   * aceso como "já iniciada" (é `existente` que acende o botão, ver a lista de Ações) e revisão e
   * histórico da análise sobreviviam. Agora a linha some da lista de verdade — exclusão lógica no
   * servidor, que preserva o rastro de auditoria sem deixar nada aparecendo na tela.
   *
   * Análise que já emitiu documento é RECUSADA pelo servidor (409): o número saiu da faixa e não
   * volta, e apagar deixaria MDP/MRP com a emissão sem a análise que a originou. A tela avisa em
   * vez de fingir que apagou.
   */
  async function limparAnalise(numeroAnalise: number) {
    const alvo = analises.find((a) => a.numero_analise === numeroAnalise);
    if (!alvo) return;
    const res = await fetch(`/api/analise-regularizacao?id=${encodeURIComponent(alvo.id)}`, { method: "DELETE" });
    const json = await res.json().catch(() => null);
    if (!res.ok || !json?.ok) {
    ...
    if (analiseAtual?.numero_analise === numeroAnalise) {
      ...
      // Excluiu outra análise: recarrega a que está em tela, senão ela fica
  ...
  function aplicarSugestaoLeituraUnica() {
     ...
     * (18/09/2026, mesmo defeito corrigido no Aceite): a versão antiga preenchia `itens` só nos
     * vazios (certo), mas escrevia `fontes`/`aceites` para TODOS os ids da sugestão, inclusive
     * os que o analista já tinha respondido. Isso deixava `fontes[id] = "p2"` e
     * `aceites[id] = false` num item que nunca veio da IA — e "Recusar todas IA" apaga qualquer
     * item com `aceites === false`, então uma resposta manual do analista podia sumir numa ação
     * que nada tinha a ver com ela. Calcula ANTES de escrever quais ids realmente vão ser
     * preenchidos, e só mexe nesses. */
  ...
  async function gerarDespacho() {
    ...
    try {
       ...
       * documento, exporta o Excel" (08/09/2026, pedido do Fábio, corrigindo o entendimento
       * anterior: não é aviso pra confirmar, é automático). Respiro de 800ms — mesmo padrão já
       * usado no backup LIP+MAC (comentário logo abaixo), porque o navegador costuma engolir um
       * segundo download disparado no mesmo instante do primeiro.
       */
      if (capturedAnaliseId) {
        setTimeout(() => {
          window.open(`/api/mac/exportar-mac?analiseId=${capturedAnaliseId}&codigo=${encodeURIComponent(codigo)}`, "_blank");
        }, 800);
      }
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    if (!res.ok || !json?.ok) {
                       ...
                       * protocolo (achado real, 08/09/2026: um erro de verdade aparecia como
                       * "Falha na leitura P3" genérico, escondendo se foi timeout de proxy
                       * (502/504) ou falha da própria rota). `res.status` é o único número
                       * confiável que sobra quando o servidor não manda um `erro` no corpo. */
