...
export default function MacPage() {
   ...
   *  Cópia isolada da mesma lógica do Slot 1 (regra do projeto: nunca compartilhar entre slots). */
  ...
  // análise atual. Apenas CONSULTAR uma análise (trocar de aba, carregamento
  // inicial) também popula `itens` — sem esta checagem o autosave regravava
  // esse snapshot por cima do banco sem edição nenhuma do analista.
  useEffect(() => {
    if (checklistItens.length === 0) return;
    if (analiseAtual) {
      const inalterado =
  ...
  function iniciarNovaAnalise() {
    ...
    //
    // O "não se aplica" descreve o lote e o tipo de edificação, que não mudam
    // entre análises. "Conforme"/"não conforme" é juízo sobre a prancha em
    // mãos, e a prancha da análise nova é outra: herdar esses status leva o
    // analista a carimbar aprovação em desenho que não reexaminou.
    //
    // Antes, a análise nova era cópia integral da anterior — o que tornava
    // "análise N idêntica à N-1" o estado NORMAL, indistinguível de gravação
    // indevida. Para copiar de propósito existe o botão 📄 de cada análise.
  ...
  async function selecionarAnalise(a: any) {
    ...
    // memória, que não acompanha gravações feitas por outra aba, outro
    // dispositivo ou correção direta no banco. Abrir a aba com o objeto velho
    // e mexer em um item só bastava para o autosave regravar o snapshot
    // desatualizado INTEIRO por cima do que estava gravado.
    let alvo = a;
    try {
      const res = await fetch(`/api/analise-aceite-sei?codigo=${encodeURIComponent(codigo)}`);
      const json = await res.json();
   ...
   *
   * Substitui a cópia automática que existia em `iniciarNovaAnalise`: o atalho
   * continua disponível, mas agora é ato deliberado, com confirmação.
   */
...
{numeroAnaliseEmAndamento && (() => {
   ...
   * análise, e "quando foi concluída" tem que ser a data em que o documento saiu. Cópia isolada
   * da mesma lógica do Slot 1 (regra do projeto: nunca compartilhar código entre slots).
   */
        ...
        <div className="w-72 bg-[var(--bg-card)] border-l border-[var(--border)] p-4 flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="flex flex-col gap-1.5 mb-1">
            {[1, 2, 3, 4, 5].map((n) => {
              ...
              // botão, como aconteceu no Slot 1 em 26/08/2026). O sinal
              // confiável é numero_despacho/numero_parecer, gravado
              // atomicamente pela rota de numeração no COMMIT do despacho —
              // `status` não serve: fica em "em_andamento" pra sempre num
              // despacho normal, só muda em indeferimento.
              const jaEmitida = !!existente && !!(existente.numero_despacho || existente.numero_parecer);
              const anterior = analises.find((a: any) => a.numero_analise === n - 1);
          ...
          <button
            onClick={() => { void salvarSilencioso(); router.push(`/logradouro/${encodeURIComponent(codigo)}?voltar=${encodeURIComponent(`/analise-aceite-sei/${codigo}`)}&rotulo=${encodeURIComponent("Voltar ao MAC")}`); }}
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={() => {
                  ...
                  // documento, depois o Excel" (08/09/2026). Aviso, não trava — cópia isolada da
                  // mesma lógica do Slot 1 (regra do projeto: nunca compartilhar entre slots).
