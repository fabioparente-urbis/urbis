...
export async function gerarDespachoRegularizacao(dados: { processo: string; interessado: string; numeroDespacho: string; naoConformes: string[]; observacoes: string; analises: { numero: number; data: string; ultima?: boolean }[]; analista?: string; crea?: string; setor?: string; assinante?: { nome: string; cargo?: string; registro?: string }; }): Promise<Buffer> {
  ...
  const totalAnalises = dados.analises.length;
  dados.analises.forEach((a, idx) => {
    const label = `${a.numero}ª ANÁLISE:       ${a.data}`;
    children.push(new Paragraph({ alignment: AlignmentType.LEFT, spacing: { before: 40, after: 40 }, indent: { left: 900 }, keepLines: true, keepNext: idx < totalAnalises - 1, children: [txt(label)] }));
  });
  // Observação condicional por número de análises
  if (totalAnalises === 4) {
    children.push(new Paragraph({ spacing: { before: 80, after: 160 }, indent: { left: 900 }, children: [txt("Observação: *Caso nesta etapa não seja liberada a taxa, o processo/projeto será indeferido.", { size: 18, italics: true })] }));
  } else if (totalAnalises >= 5) {
    children.push(new Paragraph({ spacing: { before: 80, after: 160 }, indent: { left: 900 }, children: [txt("Observação: *O processo/projeto está indeferido.", { size: 18, italics: true })] }));
  }
...
export async function gerarIndeferimento(dados: { processo: string; interessado: string; analises: { numero: number; data: string; despacho?: string }[]; endereco?: string; analista?: string; crea?: string; setor?: string; observacoes?: string; assinante?: { nome: string; cargo?: string; registro?: string }; gerente?: { nome: string; cargo?: string; registro?: string }; diretora?: { nome: string; cargo?: string; registro?: string }; }): Promise<Buffer> {
  ...
  const gerenteCau = dados.gerente?.registro || "CAU A2585910";
  const gerenteCargo = dados.gerente?.cargo || "Gerência de Análise e Licenciamento de Edificações";
  const diretoraNome = dados.diretora?.nome || "Arq. Urb. Virgínia Inácio Mathias Costa";
  const diretoraCau = dados.diretora?.registro || "CAU A35072-9";
  const diretoraCargo = dados.diretora?.cargo || "Diretoria de Análise e Aprovação de Projetos – DIRAAP";
  const assinantes = [{ nome: analista, registro: crea, cargo: cargoAnalista }, { nome: gerenteNome, registro: gerenteCau, cargo: gerenteCargo }];
  ...
  children.push(new Table({ width: { size: CW, type: WidthType.DXA }, columnWidths: [quart, half, quart], borders: { top: nb, bottom: nb, left: nb, right: nb, insideHorizontal: nb, insideVertical: nb }, rows: [new TableRow({ children: [new TableCell({ borders: brd, width: { size: quart, type: WidthType.DXA }, children: [vazio(0)] }), new TableCell({ borders: brd, width: { size: half, type: WidthType.DXA }, children: [new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 300, after: 40 }, border: { top: { style: BorderStyle.SINGLE, size: 4, color: "000000", space: 1 } }, children: [txt(diretoraNome, { size: 19 })] }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 30 }, children: [txt(diretoraCau, { size: 19 })] }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 0 }, children: [txt(diretoraCargo, { size: 19 })] })] }), new TableCell({ borders: brd, width: { size: quart, type: WidthType.DXA }, children: [vazio(0)] })] })] }) as any);
  children.push(vazio(120));
...
export async function gerarArquivamento(dados: { processo: string; interessado: string; analista?: string; crea?: string; assinante?: { nome: string; cargo?: string; registro?: string }; gerente?: { nome: string; cargo?: string; registro?: string }; diretora?: { nome: string; cargo?: string; registro?: string }; }): Promise<Buffer> {
  const logoData = getLogoData();
  const analista = dados.assinante?.nome || dados.analista || "Engº Fábio Parente Martins Santos";
  const crea = dados.assinante?.registro || dados.crea || "CREA 11716/D-GO";
  const cargoAnalista = dados.assinante?.cargo || "Analista de Obras e Urbanismo";
