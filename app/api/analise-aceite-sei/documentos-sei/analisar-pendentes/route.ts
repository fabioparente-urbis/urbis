...
import { documentosVivosGeminiAtivo, documentosVivosAceiteSeiAtivo } from "@/lib/documentosSei/config";
...
/**
 * POST /api/analise-aceite-sei/documentos-sei/analisar-pendentes — Fase 8 do plano Documentos
 * Vivos (docs/URBIS_PLANO_DOCUMENTOS_VIVOS.md §6), exclusiva do Aceite SEI (Slot 2).
 *
 * REPRODUZIDA por leitura a partir da rota irmã do Slot 1 — isolamento entre slots do CLAUDE.md.
 * Só roda sob CLIQUE explícito do analista e só classifica as páginas `classificacao_pendente`
 * que a tela mandar. Devolve PROPOSTA de papel por página; quem decide se aceita é o analista.
 *
 * Atrás de DOIS interruptores: o do Organizador (Fase 2) e o do Gemini (Fase 8,
 * `urbis_config.documentos_vivos_gemini_ativo`, default desligado).
 */
...
export async function POST(req: NextRequest) {
  const [organizadorAtivo, geminiAtivo] = await Promise.all([
    documentosVivosAceiteSeiAtivo(), documentosVivosGeminiAtivo(),
  ]);
  /**
  ...
  if (!organizadorAtivo) {
    return NextResponse.json(
      { ok: false, erro: "O Organizador de PDF SEI não está ativado para o Aceite SEI." },
      { status: 403 },
    );
  }
  if (!geminiAtivo) {
  ...
  for (const pagina of paginas) {
    ...
    try {
      ...
      await registrarChamadaIA({
        modulo: "LIP", slot: "aceite_sei", operacao: "documentos_sei_paginas_ambiguas",
