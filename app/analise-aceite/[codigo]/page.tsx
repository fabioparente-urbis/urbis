...
// MAC ACEITE — espelha /analise/[codigo]/page.tsx, com tipo fixo em "ACEITE".
const TIPO_PROCESSO = "ACEITE" as const;
const API_ANALISE = "/api/analise-aceite";
const API_DESPACHO = "/api/despacho-aceite";

export default function MacAceitePage() {
  ...
  async function carregarModelos() {
    ...
    if (json.ok) {
      const filtrados = json.data.filter((m: Modelo) =>
        m.tipo_processo === null || m.tipo_processo === TIPO_PROCESSO
      );
  ...
  async function carregar() {
    ...
    const res = await fetch(`${API_ANALISE}?codigo=${encodeURIComponent(codigo)}`);
    const json = await res.json();
  ...
  async function gerarDespacho() {
    ...
    try {
      ...
      const res = await fetch(API_DESPACHO, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          processo: codigo,
      ...
      a.href = url;
      // ACEITE_{codigo}_{data}.docx
      const hoje = new Date().toISOString().slice(0, 10).replace(/-/g, "");
      a.download = `ACEITE_${codigo}_${hoje}.docx`;
      a.click();
      URL.revokeObjectURL(url);
      mostrarToast("✅ Despacho de Aceite gerado!");
    } catch {
      ...
      {modalModelo && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-600 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            <div className="mb-5">
              <h2 className="text-white font-bold text-lg mb-1">📋 Selecione o Checklist (ACEITE)</h2>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-600 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-white font-bold text-lg">📄 Gerar Despacho (Aceite)</h2>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={gerarDespacho} disabled={gerandoDespacho}
                className="flex-1 bg-blue-700 hover:bg-blue-600 disabled:opacity-50 text-white font-bold py-2.5 rounded-lg text-sm transition-colors">
            ...
            <button onClick={async () => {
              ...
              try {
                ...
                if (res.ok) {
                  ...
                  const hoje = new Date().toISOString().slice(0, 10).replace(/-/g, "");
                  link.href = url; link.download = `ACEITE_${codigo}_${hoje}_indeferimento.docx`;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-red-700 rounded-xl p-6 w-full max-w-lg">
            ...
            {[
              ...
              "Não atendimento aos requisitos legais para emissão de Aceite",
              "Edificação em APP/APM — vedada pela legislação ambiental",
              "Inadequação cadastral / matrícula incompatível",
            ].map((motivo) => (
