...
**Versão:** 30 · **Data:** 11/09/2026 · **Estado:** em implantação — **auditoria de integração das
16 fases feita em 11/09/2026 (ver v30 no histórico): 8 defeitos achados medindo contra o banco de
produção, 7 corrigidos no mesmo dia. O painel de gestão agregava 35 dos 101 processos; o gargalo
real não era a CHEADV.** Estado anterior das fases, mantido: **Fases 0, 1, 1B e 2
...
|---|---|---|
| 33 | 14/09/2026 | **Sete recursos pedidos pelo Fábio olhando a tela `/fatiador-sei` ao vivo, um lote inteiro implementado e empurrado peça por peça (pedido dele: "vai implementando e subindo").** (a) `MiniaturaPdf` novo, coluna do meio, fixa (sticky) mesmo com a lista rolando — mostra a página candidata a corte ou a primeira do item selecionado, clique amplia no visualizador grande. (b) Setas ←/→ do teclado navegam página dentro do visualizador grande, ignoradas com foco no campo "ir pra página". (c) `Esc` fecha o visualizador. (d) Classificação de cada linha da lista virou clicável: item com `papel` (peça de contêiner) edita por `<select>` restrito ao vocabulário conhecido (`ROTULO_PAPEL_PECA` — texto livre quebraria lugares que testam `papel === "classificacao_pendente"`), item de topo edita por texto livre — liga na tela o `editarPapel`/`editarTitulo` que existiam no reducer desde a Fase 6 sem nenhum botão os chamando. (e) "Exportar confirmados (N)": zip com todos os itens ✓ de uma vez (`exportarItensEmZip` novo, testado com PDF sintético — nome repetido ganha sufixo numérico, nunca sobrescreve), depois de exportar avulso esbarrar na pergunta óbvia "e se tiver várias?". (f) "Limpar fatiador": zera tudo sem forçar escolher outro arquivo na hora, com confirmação por ser destrutivo. (g) Rascunho do último fatiamento salvo POR USUÁRIO (`lib/documentosSei/rascunhoFatiador.ts`, IndexedDB chave `usuarioId`, confirmado com ele antes de codar) — auto-salvo debounced, banner "Continuar de onde parei" ao voltar pra tela vazia. Um oitavo pedido do mesmo lote (permissão de acesso ao Fatiador configurável por usuário) ficou em aberto — ele não confirmou se é pedido novo ou só contexto. `tsc` e `next build` limpos em cada etapa; `exportarItensEmZip` testado isolado. **Não testado no navegador logado** — segue exigindo sessão de Administrador. |
| 32 | 11/09/2026 | **Três recursos pedidos pelo Fábio já usando a tela `/fatiador-sei`.** (a) **Renomear o PDF exportado** — atalho `R`, campo novo `nomeExportacao` em `estadoEdicao.ts`, ação `renomear`. Só o NOME do arquivo baixado muda, nunca o título que aparece na lista; vai direto pro arquivo sem passar por `nomeArquivoAnalista`/`rotuloDoTitulo` (achado ao implementar: aquela função tenta casar o texto contra padrões conhecidos como "laudo"/"vistoria" e reescreveria um nome escolhido à mão que por acaso contivesse uma dessas palavras — o analista pediu aquele texto, verbatim). (b) **Ir direto pra uma página** no `VisualizadorPdf` digitando o número, em vez de clicar seta a seta — campo com Enter/blur, clampado ao intervalo do item. (c) **Abrir o PDF do processo inteiro em outra aba** (`Cmd/Ctrl+P`) — `blob:` local a partir do mesmo `File` já em memória, nunca sobe ao servidor; URL revogada ao trocar de arquivo. Um quarto pedido do mesmo lote — dividir uma fatia em várias partes de uma vez, com nome próprio pra cada — foi retirado pelo próprio Fábio no meio da sessão ("aprendi a separar só a primeira página... ignorar esse item"): o fluxo existente (seta pra marcar a página do corte + `N`) já resolve isolar a primeira página sem recurso novo. `tsc` e `next build` limpos. **Não testado no navegador logado** — a tela exige sessão de Administrador. |
| 31 | 11/09/2026 | **Fase 6 fecha o ciclo: o Fatiador substitui o Organizador no Slot 1 e 2, e as duas primeiras ligações com os satélites.** (a) **URBI** — `lib/documentosSei/jornadaNaPrefeitura.ts` + bloco `fluxo.jornada_na_prefeitura` no dossiê (`lib/urbi/montarDossie.ts`). Fechava a lacuna achada na auditoria: `fluxo_processo_eventos` era lida só pelo painel de admin, e o URBI não respondia "onde esse processo travou" com o fato já gravado. MEDIDO: **65 dos 82 processos ativos (79%) estão no acervo**, então na maioria dos casos a jornada real existe. Traz duração, espera por setor, idas e vindas e a comparação com a régua do acervo. (b) **BDI** — seção "Jornada do processo pela prefeitura" em `/admin/bdi`, logo abaixo de "Tempo entre início e conclusão da análise", de propósito: uma mede dentro do URBIS, a outra mede fora (o §5.3 mediu `vw_bdi_tempo_etapas` devolvendo "minutos de análise", amostras em "0 dias"). Cruza processo ativo × jornada conhecida. **Desenho dos dois ditado pelo Fábio no meio da sessão** — *"não espero 100%, quero ajuda, e não que o URBI trabalhe por mim"*: entrega fato medido ao lado da referência e para aí; nenhum texto conclui atraso, julga setor ou sugere ação, e as observações declaram que é reconstrução por data de documento, para ordem de grandeza e não calendário. (c) **Organizador removido do Slot 1 e 2**, em duas etapas, a pedido dele ("leva os 3 pro Fatiador, depois arranca"): levantamento mostrou que os dois já cortavam o PDF pela mesma rota e já alimentavam o MHD igual — a diferença real eram **três botões**, todos portados antes de remover: comparar com o LIP e gravar na ficha (só campo VAZIO, nunca sobrescreve; a ficha é relida na hora de gravar, e o Fatiador busca/grava sozinho porque é tela separada, sem o callback que o Organizador tinha), pacote vigente + manifesto (.zip) e "Analisar páginas ambíguas" (sob clique, custo estimado antes, interruptor de IA conferido ANTES de perguntar do custo). A visão entra por ação nova `aplicarVisao` no reducer, e não por setState solto, para ser desfazível e só tocar item ainda pendente e ainda não mexido. **Slot 5 não foi tocado.** Os dois componentes do Organizador ficaram no repositório de propósito — a portagem não pôde ser exercitada no navegador logado (sem credencial), então `git revert` do commit da remoção traz a tela antiga inteira de volta; apagar os arquivos é o passo seguinte, depois do Fábio confirmar os três botões. `tsc` e `next build` limpos em todas as etapas. |
| 30 | 11/09/2026 | **Auditoria de integração das 16 fases — 8 defeitos achados, 7 corrigidos.** Tudo MEDIDO contra o banco de produção, só leitura, antes de qualquer correção. (1) **O painel `/admin/analise-fluxo` agregava 35 dos 101 processos**: o PostgREST corta todo `select()` em 1000 linhas sem avisar e a tabela tem 2441 — o mesmo defeito estava na rota da Fase 13 (portão e POST) e em `scripts/fase11_medir_fluxo.mts`, então os números da Fase 11 registrados nas versões anteriores deste plano foram medidos truncados. Corrigido com leitor paginado único (`lib/documentosSei/lerEventosFluxo.ts`), num lugar só de propósito porque eram três call sites com o mesmo erro silencioso. (2) **O tempo de espera era creditado ao setor que emitiu o documento ANTERIOR**, ou seja a quem já tinha terminado, quando pertence a quem o processo estava esperando. Decisão do Fábio depois de ver os dois rankings medidos. Efeito: **a CHEADV cai de 501 para 52 dias e deixa de ser o gargalo — o gargalo real é a Secretaria Municipal de Planejamento e Urbanismo Estratégico, 296 dias de mediana em 66 dos 101 processos.** Corrige o que estava registrado como "CHEADV é o gargalo, 205d". (3) **O `setor` nunca foi normalizado**: 54 das 89 linhas do ranking vinham de um único processo, e entre as primeiras posições apareciam um endereço, uma área ("...DE PROJETO 300,00 m²"), quebra de ligadura do OCR ("E fi ciência") e órgãos alheios ao fluxo. Novo `lib/documentosSei/normalizarSetor.ts`: agrupa grafias equivalentes (a chave descarta acento, caixa, pontuação, sigla depois de traço e todos os espaços — é o que junta "E fi ciência" com "Eficiência"), colapsa nome repetido em dobro, descarta o que comprovadamente não é setor, e o ranking passa a exigir piso de 2 processos. Descarte é contado e exibido — nada some em silêncio (§3.6). Vocabulário oficial dos setores NÃO foi inventado: continua sendo §7.2, conhecimento do analista. (4) **"Retrabalho típico: 0"**, porque a mediana de uma contagem cheia de zeros dá zero — lia-se como "não há retrabalho", falso: 31 dos 101 processos voltaram, mediana 2 entre eles, máximo 5. O painel passa a dizer quantos voltaram e quanto. (5) **O cache da Fase 8 desligava a Fase 9B**: o acerto de cache retorna antes do S1/S2/S3 e é dentro do S3 que moram o prompt combinado e a gravação em `mac_sugestoes_leitura_unica` — todo PDF já lido nunca mais geraria sugestão para o MAC, as duas economias do plano se anulavam. A rota agora se cala quando falta a sugestão para aquele processo. (6) **A chave do cache era só o hash dos bytes**, ignorando slot, assunto e texto do prompt: o mesmo PDF lido no Slot 1 devolvia a extração do Slot 1 para o Slot 2 (contaminação entre slots, contra o CLAUDE.md) e editar um prompt não surtia efeito em arquivo já lido, sem tela para limpar. A chave passa a compor hash + assunto + `tipoProcesso` + hash do conteúdo do prompt — edição feita direto no banco invalida igual. (7) **`/api/lip/cache-gemini` não tinha autenticação nenhuma** (o `middleware.ts` só renova token, quem autoriza é `autenticar()` rota a rota): POST anônimo envenenava o cache que alimenta o LIP. (8) **O gatilho da Fase 14 tenderia a ficar em zero para sempre** — contava só `fatiador_correcao`, que a tela emite num único lugar ("juntar ao vizinho anterior"); passa a contar também corte e lixo, com detalhamento por tipo, e `fatiador_leitura` entrou em `TIPOS_VALIDOS` (a tela mandava esse tipo desde a Fase 7, a rota respondia 400 e o cliente engolia no `.catch()`). **NÃO corrigido, registrado como pendência**: `editarPapel`/`editarTitulo` existem em `estadoEdicao.ts` mas nenhuma tela os chama — o diálogo de correção do §4.3, que este plano chama de "o coração do aprendizado do sistema", não foi construído. Achado que NÃO era defeito: suspeitei do parser de datas e medi — 2097 de 2098 datas lidas, só "16-01-2022" falha. Achado de arquitetura aberto: `fluxo_processo_eventos` é lida só pelo painel; nada no BDI e nada no URBI conhece a tabela, e o Fatiador não alimenta MRP/MDP/MAP — contraria a regra do CLAUDE.md de disparar para todos os satélites. `tsc` e `next build` limpos. Painel não conferido no navegador logado (precisa de sessão do Fábio). 8 pendências abertas no OBS COD. |
