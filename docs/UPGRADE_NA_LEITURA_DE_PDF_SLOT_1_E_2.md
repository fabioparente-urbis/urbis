...
**Versão:** 11 · **Data:** 10/09/2026 · **Estado:** em implantação — **Fases 0, 1 e 2 concluídas,
1B parcial** (3,5 de 16 fases: **~22% concluído, 78% restante**) — **⚠️ Fase 1 aguarda migration
aplicada · 1B tem pergunta em aberto para o Fábio**
...
| 🟡 **1B — Dar visão ao classificador** | **PARCIAL em 10/09/2026.** Setor/assinante/data agora chegam a cada peça do contêiner (`lib/documentosSei/pecas.ts`) — antes só o texto do corpo. Verificado nos 2 processos com gabarito: sinais aparecem corretamente (laudo, art, matrícula, memorial...), zero mudança na classificação existente. **Não implementado**: combinar POSIÇÃO NO FLUXO como sinal (página em branco após peça identificada = continuação dela?) — investigado em detalhe no processo real 24.5.000024350-0 (container 6376909: laudo pg 112, texto do mesmo laudo em 113-115, "ANEXOS (FOTOS)" em 116, páginas escaneadas em branco 117-123, só então um documento diferente em 124) — a extensão certa do laudo é bem maior que a página única hoje reconhecida, mas página em branco não tem sinal de texto pra provar isso sozinha. **Pergunta em aberto para o Fábio** (ele pediu para decidir documento por documento quando chegasse a hora): página escaneada sem texto, logo depois de um documento identificado, sempre é continuação dele, ou pode ser o início de outro anexo sem texto também? | 1 sessão | Taxa da Fase 0 remedida, com ganho demonstrado |
