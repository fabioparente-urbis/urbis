...
**Versão:** 28 · **Data:** 11/09/2026 · **Estado:** em implantação — **Fases 0, 1, 1B e 2
...
(16 de 16 fases com trabalho real — Fase 3 abolida virou 9A/9B nesta versão, sem renumerar 10-14;
Fase 5 e 6 continuam abertas por natureza, ver §6/§7) — migrations das Fases 1, 4 e 5 (duas)
APLICADAS e confirmadas em produção; migration da Fase 8 (`documentos_ia_cache`) escrita, ainda
não aplicada — fail-safe; migrations das Fases 9A (`leitura_unica_lip_mac_ativo`) e 9B
(`mac_sugestoes_leitura_unica`) **aplicadas e confirmadas em produção em 11/09/2026**, com teste
real de ponta a ponta (2 chamadas Gemini, Slot 1 e Slot 2); migration da Fase 10
(`fluxo_processo_eventos`) **aplicada em produção e já com 2441 linhas gravadas pela carga do
acervo**; migration da Fase 13 (`interpretacao_assistida_fluxo_ativo`) escrita, **NÃO aplicada de
propósito** — nem o interruptor sozinho liga a fase, tem um segundo portão objetivo (mínimo de
processos e de dias desde a carga) que a base de hoje não cumpre nem perto — ver v28 no histórico.
Pushes feitos até o commit da Fase 13, 11/09/2026.
