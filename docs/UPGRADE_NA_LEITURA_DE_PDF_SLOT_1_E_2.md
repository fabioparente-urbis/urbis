...
**Versão:** 26 · **Data:** 11/09/2026 · **Estado:** em implantação — **Fases 0, 1, 1B e 2
...
com NÚCLEO do painel implementado** (14 de 16 fases com trabalho real —
Fase 3 abolida virou 9A/9B nesta versão, sem renumerar 10-14; Fase 5, 6 e 12 continuam abertas por
...
2441 linhas gravadas pela carga do acervo** — ver v26 no histórico. Pushes feitos até o commit das
Fases 10-12, 11/09/2026 (`8359b90`).
...
| 🔄 **12 — Alertas e painéis** | **NÚCLEO do painel consolidado implementado em 11/09/2026**: tela `/admin/analise-fluxo` (card a acrescentar na Home, ver regra da v6) + rota `/api/admin/fluxo/portfolio`, só leitura, mesmo gate `isPerfilIrrestrito` de `/admin/mhd`. Mostra processos por faixa de tempo parado, tempo típico por setor (mediana) e retrabalho típico. **Falta**: alerta DENTRO do processo (LIP), que é a outra metade desta fase — ainda não implementado. | 2 sessões | Painel apresentável à gerência |
...
| **10 e 11** ✅ | Existe demonstração de gestão para apresentar à secretaria — painel `/admin/analise-fluxo` já mostra número real do acervo carregado. Falta só o alerta dentro do processo (resto da 12). |
