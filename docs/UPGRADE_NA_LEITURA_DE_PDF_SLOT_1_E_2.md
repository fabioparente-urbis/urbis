...
**Versão:** 8 · **Data:** 10/09/2026 · **Estado:** em implantação — **Fases 0 e 2 concluídas** (2
de 16 fases: **12% concluído, 88% restante**)
...
| ✅ **0 — Medir para mirar** | **CONCLUÍDA em 10/09/2026.** Rodado `fatiarPdfSei` (zero IA, zero rede) contra **4 processos reais** fornecidos pelo Fábio (845 páginas: 25.5.000012012-9, 24.5.000024350-0, 25.28.000000868-8, 24.5.000056065-3). **A garantia dura do fatiador se confirmou: 0 de 845 páginas foi para revisão** — nenhuma página se perdeu em nenhum dos quatro. Nos 2 processos com gabarito humano (documentos já separados manualmente em arquivos com o ID SEI no nome), **17 de 18 IDs SEI bateram exatos** com o que o fatiador identificou sozinho, incluindo os contêineres genéricos ("Documentação" com várias peças dentro) reconhecidos como um evento só, corretamente — é exatamente o comportamento que a Fase 3/6 (dividir contêiner em peças) precisa herdar. O único padrão sem correspondência **se repetiu nos dois processos com gabarito**: um documento "LAUDO" (IDs 6376909 e 11155869) vem com título genérico do SEI ("Documentação") e corpo começando em "LAUDO TÉCNICO DA EDIFICAÇÃO" — frase que não está em `ASSINATURAS_CONTEUDO` (só tem busca/vistoria/foto; falta laudo). **Não implementado** — achado reportado, não corrigido, por tocar o classificador que já roda em produção no Organizador do Slot 1/2; aguardando autorização explícita do Fábio para essa mudança específica. Métrica de campo (melhor-esforço, nunca bloqueia): setor ausente em 49% dos documentos, assinante em 66%, data em 16% — consistente nos 4 processos, sem outlier. Script permanente: `scripts/fase0_medir_fatiador.mts` (flag `--detalhe` lista cada evento, para conferir contra gabarito). | 0,5 sessão | Relatório por tipo de erro, ordenado por frequência |
...
| 8 | 10/09/2026 | **4º processo real (Fábio forneceu mais um com gabarito próprio).** Confirma o padrão: 0/845 páginas perdidas nos 4 processos juntos, 17/18 IDs SEI exatos nos 2 processos com gabarito humano, e o mesmo buraco de "Laudo" reapareceu (2º processo independente, ID 11155869) — deixa de ser achado pontual e vira padrão reproduzível. Números do §6 atualizados para os 4 processos. |
