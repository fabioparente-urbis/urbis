...
**Versão:** 22 · **Data:** 11/09/2026 · **Estado:** em implantação — **Fases 0, 1, 1B e 2
...
migrations da Fase 8 (`documentos_ia_cache`) e da Fase 9A (`leitura_unica_lip_mac_ativo`) escritas,
ainda não aplicadas — ambas fail-safe. Pushes feitos até o commit da Fase 8, madrugada de
11/09/2026, a pedido explícito do Fábio antes de dormir ("se der ruim a gente reverte o commit
amanhã").
...
| **9B — Uma leitura, dois destinos: ligação e teste** | Ligar `montarPromptCombinadoLipMac` nos dois pontos reais — o botão LER PROCESSO em `ProcessoClient.tsx` e `app/api/mac/p3/route.ts` — atrás do interruptor da 9A, testar com processo real (o interruptor ligado só num processo de teste antes de valer geral), e só então considerar estender a LER ARQUIVOS INDIVIDUAIS. **Precisa do Fábio** — mexe nos dois pipelines de leitura de produção do Slot 1/2 ao mesmo tempo. | 1 sessão | Contagem de chamadas cai pela metade, testado com processo real |
