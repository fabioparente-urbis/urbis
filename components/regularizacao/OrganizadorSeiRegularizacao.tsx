...
import { rotuloDoEvento, rotuloDoPapelPeca, montarListaDaAnalise, TIPOS_DA_ANALISE_REGULARIZACAO } from "@/lib/documentosSei/rotuloAnalista";
...
export default function OrganizadorSeiRegularizacao({
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/config")
      ...
      .then((j) => {
        if (cancelado) return;
        setAtivo(!!j?.data?.documentos_vivos_regularizacao_ativo);
        // Fase 8: aviso recorrente enquanto o Gemini estiver ligado (gasta dinheiro real por
        // página) — pedido explícito do Fábio (06/09/2026): não deixar isso passar despercebido.
        // Reaparece toda vez que a tela é aberta, não só uma vez por sessão.
  ...
  async function processar(f: File) {
    ...
    try {
      ...
      const r = await fetch(`/api/analise-regularizacao/documentos-sei?processo_codigo=${encodeURIComponent(processoCodigo)}`, { method: "POST", body: fd });
  ...
  function exportarListaTxt() {
    ...
    if (soUltimaVersao) {
      ...
      const itens = montarListaDaAnalise(resultado.eventos, TIPOS_DA_ANALISE_REGULARIZACAO);
  ...
  async function analisarPendentes() {
    ...
    try {
      ...
      const r = await fetch("/api/analise-regularizacao/documentos-sei/analisar-pendentes", { method: "POST", body: fd });
              ...
              <div className="max-h-[480px] overflow-y-auto pr-1">
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    ...
                    {soUltimaVersao && montarListaDaAnalise(resultado.eventos, TIPOS_DA_ANALISE_REGULARIZACAO).map((item) => (
