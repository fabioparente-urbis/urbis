# Schema real do banco — inventário

Gerado por `scripts/extrair_schema.mts` em 2026-09-09.

PostgreSQL 17.6 on aarch64-unknown-linux-gnu, compiled by gcc (GCC) 13.2.0, 64-bit

**198 objetos** no schema `public`: 129 tabelas, 69 views.
255 funções, 46 triggers, 347 índices, 32 policies.

| objeto | tipo | linhas | RLS |
|---|---|---:|---|
| `admin_users` | tabela | 1 | ✅ |
| `alertas` | tabela | 103 | ✅ |
| `analise_itens` | tabela | 0 | ✅ |
| `analises` | tabela | 0 | ✅ |
| `analises_mac` | tabela | 124 | ✅ |
| `assinaturas` | tabela | 0 | ✅ |
| `assuntos` | tabela | 15 | ✅ |
| `auditoria_eventos` | tabela | 6660 | ✅ |
| `auditoria_eventos_backup` | tabela | 0 | ✅ |
| `auditoria_log` | tabela | 5564 | ✅ |
| `auditoria_log_backup` | tabela | 0 | ✅ |
| `auditoria_sessoes` | tabela | 0 | ✅ |
| `auditoria_sessoes_backup` | tabela | 0 | ✅ |
| `bdi_documentos_lei` | tabela | 12 | ✅ |
| `bdi_documentos_lei_backup` | tabela | 0 | ✅ |
| `bdi_lei_fragmentos` | tabela | 3026 | ✅ |
| `bdi_lei_fragmentos_backup` | tabela | 0 | ✅ |
| `bdi_snapshots` | tabela | 7 | ✅ |
| `bip_anotacoes_usuario` | tabela | 8 | ✅ |
| `bip_historico_anotacoes` | tabela | 124 | ✅ |
| `cadastro_processo` | tabela | 0 | ✅ |
| `chat_mensagens` | tabela | 0 | ✅ |
| `checklist_instancias` | tabela | 0 | ✅ |
| `checklist_item_estatistica` | tabela | 0 | ✅ |
| `checklist_items` | tabela | 4 | ✅ |
| `checklist_itens` | tabela | 4 | ✅ |
| `checklist_itens_instancia` | tabela | 0 | ✅ |
| `checklist_modelo_itens` | tabela | 0 | ✅ |
| `checklist_modelos` | tabela | 5 | ✅ |
| `checklist_respostas` | tabela | 0 | ✅ |
| `config_urbis` | tabela | 1 | ✅ |
| `decisoes` | tabela | 0 | ✅ |
| `despacho_padroes` | tabela | 0 | ✅ |
| `despachos` | tabela | 0 | ✅ |
| `diretorias` | tabela | 1 | ✅ |
| `documentos` | tabela | 0 | ✅ |
| `documentos_lidos` | tabela | 0 | ✅ |
| `documentos_processo` | tabela | 0 | ✅ |
| `equipe` | tabela | 36 | ✅ |
| `equipe_gerencias` | tabela | 0 | ✅ |
| `equipe_roles` | tabela | 35 | ✅ |
| `etapa_tempo_sessoes` | tabela | 0 | ✅ |
| `eventos` | tabela | 87 | ✅ |
| `formato_identificadores` | tabela | 4 | ✅ |
| `gerencias` | tabela | 3 | ✅ |
| `impeditivos` | tabela | 0 | ✅ |
| `limites` | tabela | 1 | ✅ |
| `lip_abas` | tabela | 32 | ✅ |
| `lip_campos` | tabela | 282 | ✅ |
| `lip_decisoes_item` | tabela | 1 | ✅ |
| `lip_jobs` | tabela | 91 | ✅ |
| `lip_processo_atual` | tabela | 2 | ✅ |
| `lip_prompts` | tabela | 10 | ✅ |
| `lip_prompts_historico` | tabela | 14 | ✅ |
| `lip_resultados` | tabela | 8 | ✅ |
| `logradouros` | tabela | 20524 | ✅ |
| `mac_bip_vinculos` | tabela | 727 | ✅ |
| `mac_checklist_itens` | tabela | 889 | ✅ |
| `mac_checklist_itens_historico` | tabela | 0 | ❌ |
| `mac_checklist_modelos` | tabela | 3 | ✅ |
| `mac_execucoes` | tabela | 6 | ✅ |
| `mac_glossario` | tabela | 3 | ✅ |
| `mac_historico` | tabela | 10588 | ✅ |
| `mac_lip_vinculos` | tabela | 147 | ✅ |
| `mac_resultados_item` | tabela | 14 | ✅ |
| `mac_resultados_revisoes` | tabela | 0 | ✅ |
| `mac_slot5_filtros` | tabela | 29 | ✅ |
| `mac_vinculos_propostas` | tabela | 0 | ❌ |
| `mdp_registros` | tabela | 69 | ✅ |
| `mhd_conteudos` | tabela | 78 | ✅ |
| `mhd_documentos` | tabela | 74 | ✅ |
| `mhd_eventos` | tabela | 128 | ✅ |
| `mhd_interpretacoes_visao` | tabela | 0 | ✅ |
| `mhd_resultados_campo` | tabela | 1623 | ✅ |
| `mhd_versoes` | tabela | 97 | ✅ |
| `mrp_calendario` | tabela | 2 | ✅ |
| `mrp_calendario_backup` | tabela | 0 | ✅ |
| `mrp_meta_historico` | tabela | 6 | ✅ |
| `mrp_pontuacao` | tabela | 4 | ✅ |
| `mrp_pontuacao_backup` | tabela | 1 | ✅ |
| `mrp_pontuacao_historico` | tabela | 7 | ✅ |
| `mrp_registros` | tabela | 135 | ✅ |
| `mrp_registros_backup` | tabela | 33 | ✅ |
| `notificacoes` | tabela | 0 | ✅ |
| `obs_cod` | tabela | 52 | ✅ |
| `papeis_ativos` | tabela | 3 | ✅ |
| `porte_config` | tabela | 1 | ✅ |
| `processo_checklist_itens` | tabela | 0 | ✅ |
| `processo_documento_ingestao` | tabela | 0 | ✅ |
| `processo_etapas` | tabela | 0 | ✅ |
| `processo_eventos` | tabela | 87 | ✅ |
| `processo_fila_overrides` | tabela | 0 | ✅ |
| `processo_historico` | tabela | 4 | ✅ |
| `processo_prazo_interessado` | tabela | 0 | ✅ |
| `processo_profissionais` | tabela | 32 | ✅ |
| `processo_tempo` | tabela | 87 | ✅ |
| `processos` | tabela | 87 | ✅ |
| `profissionais` | tabela | 26 | ✅ |
| `profissionais_backfill_execucoes` | tabela | 2 | ✅ |
| `rh_log` | tabela | 4 | ✅ |
| `solicitacoes_despacho_extra` | tabela | 0 | ✅ |
| `solicitacoes_etapa6` | tabela | 0 | ✅ |
| `tipos_documento` | tabela | 7 | ✅ |
| `urbi_atendimento_ativo` | tabela | 1 | ✅ |
| `urbi_comandos_voz` | tabela | 10 | ✅ |
| `urbi_config` | tabela | 8 | ✅ |
| `urbi_historico` | tabela | 131 | ✅ |
| `urbi_legislacao` | tabela | 0 | ✅ |
| `urbi_presenca_eventos` | tabela | 12 | ✅ |
| `urbi_radar_execucoes` | tabela | 5447 | ✅ |
| `urbi_radar_retratos` | tabela | 1008 | ✅ |
| `urbi_regras_bloqueio` | tabela | 7 | ❌ |
| `urbi_sugestoes` | tabela | 12 | ❌ |
| `urbis_api_calls` | tabela | 100 | ✅ |
| `urbis_aportes` | tabela | 2 | ✅ |
| `urbis_config` | tabela | 1 | ✅ |
| `urbis_lip_cores` | tabela | 12 | ✅ |
| `urbis_lip_documentos` | tabela | 4 | ✅ |
| `urbis_lip_evento_tipos` | tabela | 12 | ✅ |
| `urbis_lip_eventos` | tabela | 11 | ✅ |
| `urbis_lip_fluxo_etapas` | tabela | 6 | ✅ |
| `urbis_lip_indice` | tabela | 2 | ✅ |
| `urbis_logs` | tabela | 0 | ✅ |
| `urbis_notificacoes` | tabela | 0 | ✅ |
| `urbis_numeracao_faixas` | tabela | 5 | ✅ |
| `urbis_numeracao_uso` | tabela | 81 | ✅ |
| `urbis_sessoes` | tabela | 1513 | ✅ |
| `urbis_sessoes_backup` | tabela | 0 | ✅ |
| `usuarios` | tabela | 6 | ✅ |
| `mrp_painel_diario` | view | — | — |
| `v_diretoria_mes` | view | — | — |
| `v_documento_vigente_por_tipo` | view | — | — |
| `v_equipe_publica` | view | — | — |
| `v_fila_por_analista` | view | — | — |
| `v_lip_documentos` | view | — | — |
| `v_lip_documentos_ativos` | view | — | — |
| `v_lip_etapa_atual` | view | — | — |
| `v_lip_indice_documentos` | view | — | — |
| `v_lip_indice_json` | view | — | — |
| `v_lip_indice_preview` | view | — | — |
| `v_lip_lista` | view | — | — |
| `v_lip_lista_processos` | view | — | — |
| `v_lip_painel` | view | — | — |
| `v_lip_processo_aberto` | view | — | — |
| `v_lip_processo_ativo_auto` | view | — | — |
| `v_lip_processo_em_tela` | view | — | — |
| `v_lip_processo_em_tela_com_decisao` | view | — | — |
| `v_lip_processo_interface` | view | — | — |
| `v_lip_processos_com_itens` | view | — | — |
| `v_lip_responsabilidade` | view | — | — |
| `v_lip_responsabilidade_humana` | view | — | — |
| `v_lip_status_final` | view | — | — |
| `v_lip_status_final_v2` | view | — | — |
| `v_lip_status_processo` | view | — | — |
| `v_lip_tempo_processo` | view | — | — |
| `v_lip_timeline_auto` | view | — | — |
| `v_lip_timeline_preview` | view | — | — |
| `v_lip_ultimo_documento` | view | — | — |
| `v_meu_mes` | view | — | — |
| `v_minha_gerencia_mes` | view | — | — |
| `v_pontuacao_mensal` | view | — | — |
| `v_processo_documento_ingestao_fmt` | view | — | — |
| `v_ranking_12_meses` | view | — | — |
| `v_ranking_mensal` | view | — | — |
| `v_tempo_etapa` | view | — | — |
| `v_tempo_medio_etapa` | view | — | — |
| `v_tempo_medio_processo` | view | — | — |
| `v_urbis_lip_eventos_fmt` | view | — | — |
| `v_urbis_lip_indice` | view | — | — |
| `v_urbis_lip_kpi` | view | — | — |
| `v_urbis_lip_limites` | view | — | — |
| `v_urbis_lip_resumo_181` | view | — | — |
| `v_urbis_lip_risco_pre_analise` | view | — | — |
| `v_urbis_lip_status` | view | — | — |
| `v_urbis_lip_status_v2` | view | — | — |
| `v_urbis_lip_timeline` | view | — | — |
| `vw_bdi_aguardando_retorno` | view | — | — |
| `vw_bdi_analises_em_andamento` | view | — | — |
| `vw_bdi_analistas_desempenho` | view | — | — |
| `vw_bdi_autores` | view | — | — |
| `vw_bdi_campos_criticos` | view | — | — |
| `vw_bdi_cobertura_satelite` | view | — | — |
| `vw_bdi_desempenho_referencia` | view | — | — |
| `vw_bdi_exigencias_por_contexto` | view | — | — |
| `vw_bdi_nao_conformidades` | view | — | — |
| `vw_bdi_numeracao_saldo` | view | — | — |
| `vw_bdi_por_analista` | view | — | — |
| `vw_bdi_por_assunto` | view | — | — |
| `vw_bdi_por_bairro` | view | — | — |
| `vw_bdi_produtividade_mensal` | view | — | — |
| `vw_bdi_resumo_geral` | view | — | — |
| `vw_bdi_retorno_por_slot` | view | — | — |
| `vw_bdi_retrabalho` | view | — | — |
| `vw_bdi_retrabalho_por_passada` | view | — | — |
| `vw_bdi_sessoes` | view | — | — |
| `vw_bdi_tempo_analista` | view | — | — |
| `vw_bdi_tempo_etapas` | view | — | — |
| `vw_timeline_processo` | view | — | — |
