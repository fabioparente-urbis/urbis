-- ============================================================
-- Migration: tags por processo + prompt P3_MAC
-- Aplicar no Supabase (SQL Editor) antes de testar as mudanças.
-- Nota: a chave era 'P2_MAC' e foi renomeada para 'P3_MAC' (2026-07-12).
-- ============================================================
...
-- 2) Prompt P3_MAC — só insere se ainda não existir (idempotente)
...
SELECT
  ...
  true
WHERE NOT EXISTS (
  SELECT 1 FROM lip_prompts WHERE chave = 'P3_MAC'
);
